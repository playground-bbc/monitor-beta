...
/**
 * [description] event add on term, send call to controller for update all resource history
 * @param  {[type]} e) [even when is click on add term]
 */
$("#productsIds").on("select2:select", function (e) {
  var data = {
    alertId: alertId,
    status: "Pending",
  };
 ...
 * @param  {[type]} e) [even when is click on delete resource]
 */
$("#social_dictionaryId,#free_words, #competitors").on(
  "select2:select",
  function (e) {
    var id = $(this).attr("id");
...
/**
 * [description] event select of select2, when change url on alert
 */
...
/**
 * [description] event select of select2, when change add url send call to controller to update history search on Paginas webs resource on alert
 */
...
$("#social_dictionaryId,#free_words, #competitors").on(
  ...
  function (e) {
    ...
    var dictionaryName =
      $(this).attr("resourceName") == "dictionaries"
        ? filter.text
        : $(this).attr("resourceName");
