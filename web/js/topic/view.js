const topicsView = Vue.component("cloud-view", {
  ...
  methods: {
    fetchNumbersResources() {
      axios
        .get(baseUrlApi + "numbers-resources" + "?topicId=" + id)
        .then((response) => {
...
const dictionariesCloud = Vue.component("cloud-dictionaries", {
  ...
  methods: {
    fetchWordsDictionaries() {
      axios
        .get(baseUrlApi + "cloud-dictionaries" + "?topicId=" + id)
        .then((response) => {
  ...
  methods: {
    fetchWordsHistory() {
      $(document).ready(function () {
        axios
          .get(baseUrlApi + "stadistics-date" + "?topicId=" + id)
          .then((response) => {
            if (
              response.status == 200 &&
              response.data.period.length &&
              response.data.seriesWords.length
            ) {
              $(document).ready(function () {
